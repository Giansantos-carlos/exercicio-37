...
const getFormatTime = template => {
  const [hours, minutes, seconds] = getTime()
  const formatTime = formatTimeUnits([hours, minutes, seconds])

  return template.split(':').map((_, index) => formatTime[index]).join(':')
    // .replace('h', formatHours)
    // .replace('m', formatMinutes)
    // .replace('s', formatSeconds)
}
...
class Clock {
  ...
  render () {
    const formattedTime = getFormatTime(this.template)
...
const counterParagaph =  document.querySelector('[data-js="paragraph"]')

const showCounterparagraph = e => {
  const currentLength = e.target.value.length
  const maxlength = e.target.getAttribute('maxlength')
  counterParagaph.textContent = `${currentLength} / ${maxlength}`
}
...
const reduce = (arr, func, accValue) => {
  let acc =  accValue
...
console.log(reduce([3,5,7], (acc, item) => acc + item, 0 ))
console.log(reduce([1,2], (acc, item) => {
  acc['Number-' + item] = item
  return acc
}, {}))
console.log(reduce([1,2], (acc, item, index) =>  acc = index))
